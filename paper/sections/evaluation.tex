...
produced eight data entries \jmh{``data entry'' is a weird way to describe units of provenance description.  How about ``input events from the system trace''?  BTW, this terminology suggests an obvious framing for the next sentence (and maybe the intro): \watprovenance{} produces relevant inputs to the black box, while Spade produces low-level activities from \emph{within} the box}: a get and set request and response on both the
...
file system \jmh{can you be more specific, e.g. ``the POSIX filesystem API''?}, Amazon S3, and Zookeeper. In \tabref{ProvSpecLengths}, we list the
...
relational database.  Some of these overheads can be avoided \jmh{mitigated}---e.g., \fluent{}
...
To measure the throughput of Redis, we have a single client perform small two-byte set requests to a single key for five seconds. To measure the throughput
...
\begin{figure}[t]
  ...
  \begin{subfigure}[b]{\columnwidth}
    \centering
    \includegraphics[width=0.7\columnwidth]{figures/redis_shim.pdf}
  ...
  \begin{subfigure}[b]{\columnwidth}
    \centering
    \includegraphics[width=0.7\columnwidth]{figures/s3_shim.pdf}
...
Redis is a lightweight system, and the time it takes to service a single \jmh{minimalist two-byte} set
