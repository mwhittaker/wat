...
We argue that there is no \emph{systematic} way to go about discovering this
...
%
\defword{\Watprovenance{}} is a novel form of data provenance that fills this
void.  Borrowing from causality, \watprovenance{} is a general-purpose
...
actual causes of a particular event, rather than a conservative
overapproximation.
...
\begin{theorem}\thmlabel{WatSubsumesWhy}
  ...
  \Min(t, Q)$ be an input which returns whether tuple $t$ is in $Q(I)$ for
  select, project, join, union relational algebra query $Q$. Then, viewing a
...
%
If $t \notin Q(I)$, then $o = \epsilon^*(s_0, Ti)$ evaluates to false and
$\Wat{Q, I, t}$ consists only of the empty trace, indicating that $\MWhy{Q, I,
t}$ is empty.
%
Otherwise $t \in Q(I)$ and $o$ evaluates to true. Consider a witness $T' \in
\Wat{M, T, i}$. $T'$ suffices to generate $o$, so the corresponding instance
$I'$ suffices to generate $t$. Moreover, because $Q$ is monotone and $T$ does
not contain any deletions, every witness $T'$ is closed under supertrace in
$T'$. Thus, $T'$ (and hence $I'$) is a minimal witness. The proof of the
