...
\jmh{precise? other techniques are principled, just less precise.}
...
\figref{MeanComment}. \systemname{} users can post status updates to their wall, and
...
% First, \whyprovenance{} can only be
...
% \Whyprovenance{} operates under the assumptions of a
% \emph{static} relational database, but 
% our Postgres database is
% not static. 
...
\jmh{This paragraph feels like a red herring. We can still use causal history and why provenance. Each is arguably helpful relative to raw log analysis, but neither in isolation is very useful. What we really want is a principled way to combine them so we can answer the question ``what specific combinations of event orderings and data dependencies lead to Bob seeing Ava's update?``}
...
Enter \watprovenance{}. \jmh{Doesn't really fit the tone. Probably drop it}
...
proper subtrace of $T'$ is also a witness of $o$ that satisfies (1)\footnote{%
  ...
  are not the same. See \exampleref{WatExampleAdds}, for example.
}.
\jmh{I find the use of input $i$ arbitrary. Why not focus simply on $o$ and the trace of all history leading to it, including $i$ as the last step? Also, you seem to be inconsistent as to whether we compute the \watprovenance{} of input $i$ or output $o$. I'd assume we really want it wrt $o$ everywhere.}

\subsection{Can I Get a Witness?}
We now provide a few more simple examples of \watprovenance{} to illustrate the
definition. In \secref{BlackBoxes}, we describe the \watprovenance{} of
...
\begin{example}\examplelabel{WatExampleAdds}
  ...
  \jmh{Seems like a different explanation of \watprovenance{} would have made this clearer--the definition of \Wit{M,T,i} is obscuring the point. You want to compute $minimal(closedUnderSuper(witnesses))$. $minimal$ does not commute with $closedUnderSuper$. Then proof by example.}
\end{example}
...
\begin{theorem}\thmlabel{Existence}
  For every state machine $M$, trace $T$, and input $i$, $\Wat{M, T, i}$ is
  non-empty.
\end{theorem}

\thmref{Existence} is a simple sanity check for our definition of
...
\jmh{Its not appropriate to reference an example in this theorem. You want a general statement. I'd define the special case of relational state machines, and a trace language of \textsf{insert} and \textsf{query} with the query language appropriately scoped. Then define the database instance at timestep $t$ as $I_t$, and prove a statement about $\Wat{M,T,i}$ and $\MWhy{Q,I_t}$}
\newcommand{\Min}{\text{in}}
\begin{theorem}\thmlabel{WatSubsumesWhy}
  Let $M$ be the relational database state machine from
  \exampleref{WatExampleKvs}. Let $T$ be a trace which inserts every tuple in
...
In short, \thmref{WatSubsumesWhy} says that \watprovenance{} subsumes
...
\jmh{What do you mean the output $o$ is ``false''?}
%
Otherwise $t \in Q(I)$ and $o$ is true. 
\jmh{What do you mean the output $o$ is ``true''?}
Consider a witness $T' \in \Wat{M, T,
i}$. $T'$ suffices to generate $o$, so the corresponding instance $I'$ suffices
to generate $t$. Moreover, because $Q$ is monotone \jmh{and your trace is monotone -- no deletes!}, every witness $T'$ is
...
\todo{Insert claim/proof saying that we subsume causality.}
