...
boolean that indicates whether tuple $t$ is in $Q(I)$. Then, viewing a subtrace
$T'$ as a subinstance $I' \subseteq I$, $\Wat{M, T, i} = \MWhy{Q, I, t}$.
...
\todo{Consider re-writing this once we re-write section 5.}
Note that \watprovenance{} returns a set of witnesses that are local to a
particular node. On its own, \watprovenance{} does not return the causal
history of these witnesses, which includes messages sent by other nodes in the
distributed system. In \secref{Fluent}, we see how to enrich \watprovenance{}
with this information.

\subsection{Limitations}
\todo{%
  Discuss the limitations of \watprovenance{}. In particular, make it clear
  that \watprovenance{} can tell you what inputs contributed to a particular
  output, but it cannot help you debug particular implementations of code and
  why certain things are happening. Mention that the state machine has to be
  non-deterministic. Mention that sometimes \watprovenance{} is confusing like
  with the insertion, deletion, append stuff?
}
