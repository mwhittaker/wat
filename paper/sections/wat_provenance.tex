...
sequence of events. \jmh{That is an odd thing to argue, as you are about to present such a way!  Rephrase to critique prior techniques, but leave open room for a new technique.} One possibility is to use causality, as described in
...
produces output $o$ \jmh{when given input $i$}. We build this intuitive notion by way of a sequence of
...
\jmh{Consider renaming these ``observations'' or lemmas.}
...
\begin{theorem}\thmlabel{WatSubsumesWhy}
  ...
  execution of \jmh{monotone} relational queries \jmh{: queries over the monotone relational algebra operators select, project, join and union}. Let $I$ be an arbitrary database instance,
  ...
  $t$ is in $Q(I)$ 
  \jmh{drop the remainder of sentence}
  for select, project, join, union relational algebra query $Q$. 
  Then, viewing a subtrace $T'$ as a subinstance $I' \subseteq I$,
