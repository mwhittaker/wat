...
\watprovenance{} specifications for all the components in \figref{MeanComment}. \jmh{Can you prove that writing those is reasonable? E.g. did you do it, and can you say how many lines it was or something?}
...
\begin{itemize}
  ...
  \item
    ...
    mean comment from application server $s_3$. \jmh{shouldn't the load balancer depend on a previous load report from an application server? where did that dependency go in this trace?}
  \item
  ...
  \item
    ...
    previously inserted into the database by application server $s_3$. \jmh{$s_2$??}
  \item
