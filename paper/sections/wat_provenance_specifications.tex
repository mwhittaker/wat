...
There are two conditions under which a \watprovenance{} specifications returns
incorrect provenance for a particular state machine: the specification is
incorrect or the state machine is incorrect. We have thus far tacitly
assumed that common black boxes like Redis are correct and faithfully implement
their advertised interfaces.  However, if a black box is buggy and deviates
from its expected behavior, then a \watprovenance{} specification will produce
erroneous provenance. Fortunately, black box bugs are relatively rare (at least
compared to the frequency of bugs in application-specific code written from
scratch).
%
