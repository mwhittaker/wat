...
\jmh{You say better here what you said earlier. Figure out how to remove redundancy and keep flow good.}
...
\watprovenance{} of any request consists only of the empty witness. \jmh{The AI examples are problematic ... answers vary over time because model training incorporates feedback. Today this is typically at a long timescale via offline training software like tensorflow embedded in some workflow for model updates. But I think everybody knows that AI software ``learns over time'' and they'll critique you for this.}
...
\jmh{Here you may get critiques about distributed systems: the non-determinism of eventual consistency, or alternative semantics to last-writer-wins. At minimum you have yet to tell us how to think about distributed state machines. I *think* you want to tell us that each node is a black box of its own .. but let's talk.}
...
previously in the bucket. \jmh{I think you need to be more careful discussing move.}
...
overlap with the requested byte range.  \jmh{I think you need to be more careful discussing rename/mv. Also links. And then there's the replica consistency issues mentioned above.}
...
the znode and the most recent creations of all ancestor znodes. \jmh{It's important that Zookeeper has linearizable write semantics (doesn't it?) so you can use your sequential trace reasoning.}
...
\jmh{I like this. Let's keep this discussion here, and any uses of this reasoning should be references to this section.}
...
system. 
\jmh{I think the below is a red herring. The filesystem abstraction of Zookeeper has very little to do with the important causality and data-dependency assumptions a user is likely to make. If it were a KV API users would probably make the same assumptions.}
...
\jmh{Careful! You snuck delete in here, which you ignored in the theorem. I think you could add it to the theorem BTW.}
...
\jmh{I don't see how the following is a response to the preceding. DSL APIs like SQL are just too expressive to admit simple specs, even by an expert. Instead, we probably want to push the task of writing specs a level up -- in the service/application that is creating specific SQL expressions.}
...
\jmh{You express this in terms of a formalism but I think you want the reader to think about it in terms of a practical system. Consider rewording.}
...
impractical for computing the \watprovenance{} of real-world traces. \jmh{Perhaps reference the ``small-scope hypothesis''?}
...
\jmh{This strikes me odd. There's an implicit assumption that the spec is correct but the box is buggy. Be more explicit on that front here.  Maybe organize this entire section to have three ways that spec and implementation diverge: (a) limitations of the spec in capturing the semantics of the box, (b) bugs in a box that was intended to meet the spec, or (c) non-determinism in the box makes any single spec insufficient (could we write a disjunctive spec??).  We should discuss.}
...
\jmh{You just finished telling us this was impractical.}
