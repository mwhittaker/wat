...
another file, then the \watprovenance{} specification \jmh{also includes the provenance of the file that was moved.} may also include writes
to the moved file.
...
\jmh{I find that this paragraph is an unsatisfying response to the prior paragraph -- it really doesn't help with the case of relational databases or other ``open-ended'' APIs involving nested DSLs.  I'd decouple the 2 points below from the example above, and respond to the example above with a different argument.}
...
real-world traces.
% \jmh{Perhaps reference the ``small-scope hypothesis''?}
...
implement their advertised interfaces. \jmh{I'm still unhappy with this framing. The issue you're really describing here is whether the \watprovenance{} spec is faithful to the implementation. If not there are two possible problems: the spec is wrong, or the implementation is wrong (e.g., has unintended bugs).} However, if a black box is buggy and
