...
inputs of another. 
\jmh{I'm a bit confused here. In discussing relational provenance we talked about a static input database. Here you're talking somehow about a static computation. The analogy doesn't work. I think the analogy is to relational algebra, not staticness: these folks do lineage w.r.t. their workflow langauges, but those are not typical programming models for distributed systems.}
...
\jmh{I'd say this is (a) limited language and (b) fixed input. Basically the same worries as relational, but with different DSLs than SQL.}
...
requests wrote the values that later requests read back. \jmh{Really? Dapper can't give you causal traces??}
